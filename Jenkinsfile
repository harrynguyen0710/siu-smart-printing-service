pipeline {
    agent any
    environment {
        REPO_URL = 'https://github.com/harrynguyen0710/siu-smart-printing-service.git'
        ...
        DOCKER_IMAGE = 'tanloc27/printing:latest'  // Docker image name with version tag
            ...
            steps {
                script {
                    try {
                        ...
                        docker run -d -p 80:80 -e ASPNETCORE_URLS="http://0.0.0.0:80" --name webserver --network network ${DOCKER_IMAGE}
                        """
